...
export class SessionManager {
	...
	private scheduleRefresh(session: SupabaseSession): void {
		...
		const expiresAt =
			session.expires_at || Date.now() + expiresInSeconds * 1000;
	...
	private async setSession(session: SupabaseSession): Promise<void> {
		if (!session || !session.user || !session.user.id) {
			console.error("Invalid session data received, clearing session");
			this.clearSession();
			this.emit("session_cleared");
			return;
		}
	...
	public getSessionInfo(session?: SupabaseSession): SessionInfo | null {
		...
		const expiresInSeconds = currentSession.expires_in ?? 3600;
		const expiresAt =
			currentSession.expires_at || Date.now() + expiresInSeconds * 1000;
		const now = Date.now();
