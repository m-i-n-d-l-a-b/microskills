...
const envSchema = z.object({
  // API Configuration
  VITE_GRAPHQL_ENDPOINT: z.string().url().default('http://localhost:4000/graphql'),
  ...
  VITE_LLM_TIMEOUT: z.string().regex(/^\d+$/).transform(Number).default('30000'),
  VITE_LLM_MAX_RETRIES: z.string().regex(/^\d+$/).transform(Number).default('3'),
...
export function validateEnv(): EnvSchema {
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      console.warn('Continuing with default values for missing/invalid environment variables');
    }
 ...
 */
export function validateProductionEnv(): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (import.meta.env.PROD) {
    ...
    if (import.meta.env.VITE_SUPABASE_URL && !import.meta.env.VITE_SUPABASE_ANON_KEY) {
      errors.push('VITE_SUPABASE_ANON_KEY is required when VITE_SUPABASE_URL is set');
    }
  }
